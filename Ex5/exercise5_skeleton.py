...
def average_one_hots(sent, word_to_ind):
    ...
    indices = [word_to_ind[w] for w in sent.text if w in word_to_ind]
    avg = np.zeros(vocab_size, dtype=np.float32)
    if len(indices) == 0:
        return avg
    for ind in indices:
        avg += get_one_hot(vocab_size, ind)
    avg /= len(indices)
    return avg
...
class LogLinear(nn.Module):
    ...
    def forward(self, x):
        return self.linear(x).squeeze(-1)   # logits, shape (batch,)
...
def binary_accuracy(preds, y):
    ...
    :return: float
    """
    # Convention: callers pass probabilities (after sigmoid), so we threshold at 0.5.
...
def train_epoch(model, data_iterator, optimizer, criterion):
    ...
    for x, y in data_iterator:
        ...
        total_acc  += binary_accuracy(torch.sigmoid(logits), y) * batch_size
        n += batch_size
...
def train_log_linear_with_one_hot():
    ...
    input_dim = data_manager.get_input_shape()[0]      # = vocab size

    model = LogLinear(input_dim)
    history = train_model(model, data_manager, n_epochs=10, lr=0.01, weight_decay=0.0)

    plot_and_save(history, "Log-Linear (one-hot)", "log_linear_onehot")
