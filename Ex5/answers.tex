...
\paragraph{Clarifications established along the way.}
\begin{itemize}
\item ``Parameters of a matrix'' = its individual entries; a $d \times d$ matrix
has $d^2$ parameters, each a knob that training adjusts.
\item Each weight matrix defines a linear map applied per token: $W_Q, W_K, W_V,
W_O$ map a token's $d$-vector to a $d$-vector (hence $d \times d$), and the
embedding matrix is the weight of a linear map from a $V$-dim one-hot vector to
a $d$-dim dense vector (multiplying by a one-hot just selects a row).
\item The output $y = Wx$ has no parameters - it is recomputed per input; only
$W$ is learned.
\item $n$ (context length) is a property of the \emph{input} and can vary per
input, unlike the fixed architecture choices $V, d, L$.
\item A maximum context length is a real model limitation but adds no parameters
here; the one term that \emph{would} make $n$ matter is positional embeddings,
which the question explicitly ignores.
\item $W$ acts on one token's $d$-vector at a time, so $n$ is how many times $W$
is reused, not its shape; $n$ and $d$ are not comparable dimensions ($d$ is a
single token's width, $n$ is the token count).
\item $W_O$ is the output projection: the final $d \times d$ map applied after
attention mixes the values, recombining the result back into dimension $d$
...
with a fixed parameter budget. (If asked to expand $R$ for a vanilla RNN with
hidden size $h$, it would be roughly $R = d h + h^2$ ignoring biases - an
input$\to$hidden matrix plus a hidden$\to$hidden matrix; the question spares us
this by giving $R$ directly.)
...
\medskip
\noindent\textbf{Punchline.} Growing $n$ by $5\times$ costs \emph{zero}
parameters (only runtime compute), while shrinking $V$ saves a large amount.
Character-level tokenization makes both models \emph{smaller} in parameter
count - the trade-off is paid in sequence length (compute), not in weights.
