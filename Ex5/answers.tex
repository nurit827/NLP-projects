...
length rather than in weights. And for an autoregressive model that cost is not
only extra computation but also extra \emph{time}: since such a model produces
tokens one at a time, a $\sim$5$\times$ longer sequence means $\sim$5$\times$
more sequential generation steps that cannot be parallelized away, on top of the
higher per-step compute (such as the $n^2$ attention) that the longer context
adds anyway.
